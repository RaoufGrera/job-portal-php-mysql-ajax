...
jQuery(function($) {
	...
	$("form input[name='register_comp']").click(function() { // triggred click 
		...
		var comp_name = jQuery.trim($("form input[name='comp_name']").val()); // last name field
		...
		if(comp_name == "") {
			$("span.comp_name_val").html("يجب تعبئة الحقل .");
		val_holder = 1;}
		else{
		$("span.comp_name_val").html("");}
		...
		var datastring = 'fname='+ fname +'&lname='+ lname +'&email='+ email+'&txt_pass='+txt_pass+'&comp_name='+comp_name; // get data in the form manual
		//var datastring = $('form#mainform').serialize(); // or use serialize

		$.ajax({
					type: "POST", // type
					url: "check_email_emp.php", // request file the 'check_email.php'
					...
					success: function(responseText) { // get the response
						 if(responseText == 2) { // if the response is 1
							...
							if(responseText == "") { 
								$("span.loading").html("<img src='images/correct.png'> قد تم تسجيلك بنجاح");
								$("span.validation").html("");
