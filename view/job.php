...
$id_desc=safe_input($id);
                         $rs=$mysqli->query("SELECT job_description.emp_id,comp_active,specialty,age_min,age_max,salary_name,status_name,see_it,job_employer.comp_id,job_skilles, job_name, job_description.desc_id,job_start,job_end,comp_name,job_desc,job_gender,domain_name,exp_max,exp_min,job_num,email,phone,address,type_name,edt_name,health_status,nat_name,city_name,comp_active,image,comp_type_name,last_seen,exp_type_name
				 ...
				 if(!empty($txt_type)){
					 if ($txt_type== 'seeker'){
						if($saved==1){
						 ...
						 echo "<a  href='javascript: submitform()' ";
				   ...
				   <img  src='../images/unsave.png'/><a  name='req' onclick="return confirm('يجب عليك التسجيل كباحث عن عمل لحفظ هذه الوظيفة')"   href="../seeker.php"  ><span class='textsss' > حفظ هذه الوظيفة</span> </a><p>
				  <?php
				  }?>
...
<tr >
	...
	<span class="texttt"><?php echo "الحد الأدني: ".$row->exp_min;
	if($row->exp_max > $row->exp_min){
		
		echo " الحد الأقصي : ".$row->exp_max; 
	}
	...
	<span class="texttt"><?php echo "الحد الأدني: ".$row->age_min;
	if($row->age_max > 0){
		echo " الحد الأقصي: ".$row->age_max;
