...
$delete_id= safe_input($_GET['delete_id']);
	...
	//حذف الموظف
	$rsdelete = $mysqli->query("DELETE FROM `job_company` WHERE   comp_id= '$delete_id'" ); 
...
<ul id="tabnav">
<li class="se"><a href="companys.php">الشركات</a></li>
...
<li ><a href="jobs.php">الوظائف</a></li>
   ...
   <div>		
		...
		switch ($msg) {
		case "deleted":
		echo "<p class='truee'><img width='13' height='13' src='../images/tick.png'/>قد تم حذف الشركة </p>";
		break;
		case "msg_false":
		echo "<p class='error'><img width='13' height='13' src='../images/invalid.png'/>حدث خطاء  أثناء التنفيذ</p>";
		break;
		case "block":
		echo "<p class='truee' ><img width='13' height='13' src='../images/tick.png'/>قد تم حظر الشركة</p>";
		break;
		case "unblock":
		echo "<p class='truee' ><img width='13' height='13' src='../images/tick.png'/>قد تم إلغاء حظر الشركة</p>";
		break;
		default:
...
<form id="form1" name="form1" method="GET" action="companys.php">
<div class='search'>
<label>الأسم</label>
...
if (!empty($_GET["string"])) {
	$search_string = " and (comp_name LIKE '%".mysql_real_escape_string($_GET["string"])."%'  OR job_company.comp_id LIKE '%".mysql_real_escape_string($_GET["string"])."%')";	
...
$records_at_page = 20;
$sql_result_row = "SELECT * FROM job_company,job_city,job_employer 
							WHERE job_employer.comp_id = job_company.comp_id AND job_city.city_id = job_company.city_id  
							AND level = 1  ";
$sql_result_row.=$search_string ;
$sql_result_row.=$search_city ;
$sql_result_row.= " group by job_company.comp_id " ;
...
$sql = "SELECT job_company.comp_id,comp_name,job_company.block_admin,emp_id,fname,email FROM job_company,job_city,job_employer 
							WHERE job_employer.comp_id = job_company.comp_id AND job_city.city_id = job_company.city_id 
							AND level = 1 ";
$sql.=$search_string ;
$sql.=$search_city ;
$sql.= " group by job_company.comp_id " ;
...
?>
 ...
 <?php  
	...
	echo '<td><a  href="../view/viewcompany.php?id='.$row['comp_id'].'">'.$row['comp_name'].'</a></td>';
	echo "<td>".$row['email']."</td>";
	echo '<td><a  href="employers.php?string='.$row['emp_id'].'">'.$row['fname'].'</td>';


	echo "<td>";
		   
   echo '<a href="companys.php?id='.$row['comp_id'].'&block='.$row['block_admin'].'">';
	...
	echo '<a onclick="return confirm(\'هل أنت متأكد من الحذف ?\')"   href="companys.php?delete_id='.$row['comp_id'].'" ><img src="../images/remove.png"/> حذف</a>';
	echo "</td>";   
	echo "</tr>";
